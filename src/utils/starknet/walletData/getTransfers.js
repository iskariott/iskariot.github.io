...
async function fetchTransfers(url, Json_data, headers, prices) {
  ...
  response.data.data['erc20TransferEvents']['edges'].forEach((item) => {
    ...
    transfers.push({
      ...
      main_call,
      __typename,
      total_value: prices.hasOwnProperty(from_erc20_identifier)
        ? transfer_amount_display * prices[from_erc20_identifier]
        : 0,
    });
  });
  return {
...
export async function getTransfers(address, prices) {
  ...
  let allTransfers = [];
  let results = await fetchTransfers(url, Json_data, headers, prices);
  allTransfers.push(...results.transfers);
  while (results.hasNextPage) {
    Json_data['variables']['after'] = results.endCursor;
    results = await fetchTransfers(url, Json_data, headers, prices);
    allTransfers.push(...results.transfers);
  }
  return allTransfers;
}
